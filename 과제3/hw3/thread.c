    ...
    */
    ...
    while(1) {
        ...
        if (pThreadTbEnt[tid].pThread->status == THREAD_STATUS_ZOMBIE) {
            //printf("\t\t\t\tjoin() : zombie status thread\n");
            ...
            // �߿� !!!
            //if (ReadyQueue.pHead != NULL && ReadyQueue.queueCount == 0) ReadyQueue.queueCount++;
            ReadyQueue.queueCount++;
        ...
        // ������
            ...
            */
            ...
            int curpid = getpid();
            int newpid = ReadyQueue.pHead->pid;

            if (ReadyQueue.queueCount == 1) {
                pCurrentThread = ReadyQueue.pHead;
                pCurrentThread->status = THREAD_STATUS_RUN;
                ReadyQueue.pHead = NULL;
                ReadyQueue.pTail = NULL;
            ...
            //printf("thread_join() : pause() : %d\n", getpid());
            
            pause();
            //printf("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tWake up pause()\n");
